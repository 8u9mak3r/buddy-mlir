...
def _gen_arith_binary_op(input1, input2, op_func):
    ...
    type1 = ir.RankedTensorType(input1.type).element_type
    result_element_type = ir.IntegerType.get_signless(32) if str(type1) == 'i8' and 'mul' in str(op_func) else type1
    result_tensor_type = ir.RankedTensorType.get(
        broadcasted_result_shp, result_element_type
    )
    op = op_func(result_tensor_type, input1, input2)
        ...
        if is_depthwise is True:
            ...
            # Generic Conv2D Operation.
                ...
                if dtype == TensorDType.Int8:
                    ...
                    if not all(x == 0 for x in input_padding):
                        pad_values_type = ir.RankedTensorType.get(
                            [4, 2], ir.IntegerType.get_signless(32))
                        pad_values = ir.DenseElementsAttr.get(
                            numpy.array(
                                [[0, 0],
                                [input_padding[0], input_padding[1]],
                                [input_padding[2], input_padding[3]],
                                [0, 0]], dtype=numpy.int32
                            ),
                            type=pad_values_type
                        )
                        padding = arith.ConstantOp(pad_values_type, pad_values).result
                        ...
                        perm_shape.append(input_shape[3])
                        perm_shape.append(input_shape[1])
                        low = [0, input_padding[0], input_padding[1], 0]
                        high = [0, input_padding[2], input_padding[3], 0]
                        pad_shape = list(map(lambda a, b, c: a + b + c, perm_shape, low, high))
                        pad_result = ir.RankedTensorType.get(pad_shape, ir.IntegerType.get_signless(8))
                        # print(str(pad_shape))
                    
                        input_val = tosa.PadOp(pad_result, input_val, padding).result
